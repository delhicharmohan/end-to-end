    ...
    <Transition v-if="isPermission" name="fade" mode="out-in">
      ...
      <section key="2" v-else class="h-full">
        ...
        >
              ...
              <div class="inline-block min-w-full sm:px-6 lg:px-8">
                <div class="overflow-x-auto">
                  <table class="min-w-full text-left text-sm font-light">
                    ...
                    <tbody>
                      <tr
                        v-for="(orderCreator, index) in filteredUsers"
                        :key="index"
                      ...
                      >
                        ...
                        <td>
                          <div class="flex justify-center items-center">
                            <icon-edit
                              @click.stop="updateUser(orderCreator)"
                              class="h-5 w-5 cursor-pointer hover:text-blue-600 mr-2"
                            ></icon-edit>
                            <icon-delete
                              @click.stop="deleteUser(orderCreator)"
          ...
          <h3 v-else class="text-center font-medium">No order creator users found!</h3>
        </base-card>
      </section>
    </Transition>
...
import UserForm from "../../components/order-creator/UserForm.vue";
...
export default {
  ...
  computed: {
    ...
    filteredUsers() {
      return this.$store.getters["ordercreator/users"];
    },
    hasUsers() {
      return this.$store.getters["ordercreator/hasUsers"];
    },
    getFormText() {
      if (this.mode == "create") {
        return "Create Order Creator";
      } else {
        return `Update Order Creator - ${this.selectedUser.username}`;
      }
    },
  },
  created() {
  ...
  methods: {
    async getUsers() {
      this.isLoading = true;
      try {
        await this.$store.dispatch("ordercreator/getUsers");
      } catch (error) {
        if (!error.status) {
    ...
    async createUser(data) {
      ...
      try {
        if (this.mode == "create") {
          await this.$store.dispatch("ordercreator/createUser", data);
          this.baseToast.message = "Order Creator User created successfully!";
        } else {
          await this.$store.dispatch("ordercreator/updateUser", data);
          this.baseToast.message = "Order Creator User updated successfully!";
        }
        ...
        .then(async (result) => {
          if (result.isConfirmed) {
            await this.$store.dispatch(
              "ordercreator/deleteUser",
              this.selectedUser
            );
            this.$swal.fire(
              "Deleted!",
              "Order Creator User has been deleted.",
              "success"
            );
            this.selectedUser = Object;
            this.isCreateUser = false;

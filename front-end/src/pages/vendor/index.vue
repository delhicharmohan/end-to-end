    ...
    <Transition v-if="isPermission" name="fade" mode="out-in">
      ...
      <section key="2" v-else class="h-full">
        ...
        >
              ...
              <div class="inline-block min-w-full sm:px-6 lg:px-8">
                <div class="overflow-x-auto">
                  <table class="min-w-full text-left text-sm font-light">
                    ...
                    <tbody>
                      <tr
                        v-for="(vendor, index) in filteredUsers"
                        :key="index"
                      ...
                      >
                        ...
                        <td>
                          <div class="flex justify-center items-center">
                            <icon-edit
                              @click.stop="updateUser(vendor)"
                              class="h-5 w-5 cursor-pointer hover:text-blue-600 mr-2"
                            ></icon-edit>
                            <icon-delete
                              @click.stop="deleteUser(vendor)"
          ...
          <h3 v-else class="text-center font-medium">No vendor found!</h3>
        </base-card>
      </section>
    </Transition>
...
import UserForm from "../../components/vendor/UserForm.vue";
...
export default {
  ...
  computed: {
    ...
    hasUsers() {
      return this.$store.getters["vendor/hasUsers"];
    },
    getFormText() {
      if (this.mode == "create") {
        return "Create Vendor";
      } else {
        return `Update Vendor - ${this.selectedUser.username}`;
      }
    },
  },
  created() {
  ...
  methods: {
    ...
    async createUser(data) {
      ...
      try {
        if (this.mode == "create") {
          await this.$store.dispatch("vendor/createUser", data);
          this.baseToast.message = "Vendor created successfully!";
        } else {
          await this.$store.dispatch("vendor/updateUser", data);
          this.baseToast.message = "Vendor updated successfully!";
        }

    ...
    <Transition v-if="isPermission" name="fade" mode="out-in">
      ...
      <section key="2" v-else class="h-full">
        ...
        >
              ...
              <div class="inline-block min-w-full sm:px-6 lg:px-8">
                <div class="overflow-x-auto">
                  <table class="min-w-full text-left text-sm font-light">
                    ...
                    <tbody>
                      <tr
                        v-for="(supportUser, index) in filteredUsers"
                        :key="index"
                      ...
                      >
                        ...
                        <td>
                          <div class="flex justify-center items-center">
                            <icon-edit
                              @click.stop="updateUser(supportUser)"
                              class="h-5 w-5 cursor-pointer hover:text-blue-600 mr-2"
                            ></icon-edit>
                            <icon-delete
                              @click.stop="deleteUser(supportUser)"
          ...
          <h3 v-else class="text-center font-medium">No support user found!</h3>
        </base-card>
      </section>
    </Transition>
...
import UserForm from "../../components/support-user/UserForm.vue";
...
export default {
  ...
  computed: {
    filteredUsers() {
      return this.$store.getters["supportuser/users"];
    },
    hasUsers() {
      return this.$store.getters["supportuser/hasUsers"];
    },
    getFormText() {
      if (this.mode == "create") {
        return "Create Support User";
      } else {
        return `Update Support User - ${this.selectedUser.username}`;
      }
    },
  },
  created() {
  ...
  methods: {
    async getUsers() {
      this.isLoading = true;
      try {
        await this.$store.dispatch("supportuser/getUsers");
      } catch (error) {
        if (!error.status) {
    ...
    async createUser(data) {
      ...
      try {
        if (this.mode == "create") {
          await this.$store.dispatch("supportuser/createUser", data);
          this.baseToast.message = "Support User created successfully!";
        } else {
          await this.$store.dispatch("supportuser/updateUser", data);
          this.baseToast.message = "Support User updated successfully!";
        }
        ...
        .then(async (result) => {
          if (result.isConfirmed) {
            await this.$store.dispatch(
              "supportuser/deleteUser",
              this.selectedUser
            );
            this.$swal.fire(
              "Deleted!",
              "Support User has been deleted.",
              "success"
            );
            this.selectedUser = Object;
            this.isCreateUser = false;

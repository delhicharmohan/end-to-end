    ...
    <div class="h-full flex flex-col p-4">
        ...
        <section key="2" class="h-full flex flex-col">
            <base-card class="h-full flex flex-col">
                ...
                > -->
                        <base-button v-if="!showSearch" mode="outline"
                            @click.stop="searchBtnClicked">Search</base-button>
                    ...
                    <div :class="isAdmin ? 'grid-cols-4' : 'grid-cols-3'" class="w-full grid gap-8 mt-4 px-4">
                        ...
                        <div v-if="isAdmin" class="flex flex-col">
                            <select v-model.trim="validatorUsername">
                                ...
                                <option v-for="(agent, index) in agents" :key="index" :value="agent.username">{{
                                    agent.username }}
                                </option>
                            </select>
                        </div>
                ...
                <div class="flex px-4 my-4">
                    <div class="flex justify-start items-center">
                        ...
                        <label for="select-all" class="ml-2 mb-0 cursor-pointer">{{ selectAll ? 'Unselect All' :
                            'SelectAll'
                            }}</label>
                    </div>
                </div>
                ...
                <div v-else-if="hasPayOutList && !isLoading"
                    ...
                    <div class="grid grid-cols-1 gap-4">
                        <pay-out-item v-for="(item, index) in filteredPayOutList" :key="index" :item="item"
                            :sl-no="paginatorInfo.firstItem + index" :select-all="selectAll"
                            :clear-selected="clearSelected" @select-item="selectItem"></pay-out-item>
                    </div>
        ...
        <transition name="fade" mode="out-in">
            <base-modal v-if="createPayOutOrderModal" title="Create Pay Out Order" @close="closeCreatePayOutOrderModal">
                ...
                <div v-else class="flex flex-col mt-8" ref="tab1">
                    ...
                    <div class="flex flex-row flex-wrap items-center mt-4 px-8"
                        :class="{ invalid: !payoutAmount.isValid }">
...
export default {
    ...
    created() {
        ...
        if (this.role == "admin" || this.role == "subadmin") {
            ...
            this.socket.on(`${this.vendor}-payout-order-amount-created`, (data) => {
                this.updatePayOutAmountAndStatus(data);
            });
        } else {
            this.socket.on(`${this.vendor}-payout-order-created-${this.username}`, (data) => {
                this.addOrderToPayOutList(data);
            });
        ...
        this.socket.on(`${this.vendor}-update-payout-link-order`, (data) => {

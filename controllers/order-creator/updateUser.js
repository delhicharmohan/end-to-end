...
async function updateUser(req, res) {
  ...
  try {
    ...
    if (result.affectedRows === 0) {
      logger.info("Order Creator User not found");
      return res.status(404).json({ message: "Order Creator User not found." });
    }

    logger.info("Order Creator User updated successfully");
    logger.debug(result);
    return res.status(200).json({ message: "Order Creator User updated successfully." });
  } catch (error) {
    logger.error(error);

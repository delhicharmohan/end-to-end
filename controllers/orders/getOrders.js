...
function buildQuery(fields, vendor) {
  ...
  if (fields.hasOwnProperty('payout_type')) {
    payoutType = fields.payout_type;
    delete fields.payout_type;
  }
  ...
  if(payoutType == 'standard') {

    if (isRemoveLastQuery) {
      query += ` AND payout_type is NULL`;
      countQuery += ` AND payout_type is NULL`;
    } else {
      query += ` WHERE payout_type is NULL `;
      countQuery += ` WHERE payout_type is NULL `;
  ...
  if(payoutType == 'instant') {
    if (isRemoveLastQuery) {
      ...
      countQuery += ` AND payout_type = ? `;
    } else {
      query += ` WHERE payout_type = ? `;
      countQuery += ` WHERE payout_type = ? `;
      searchKeyWordCondition = null;
    }
    data.push(payoutType);
  }
